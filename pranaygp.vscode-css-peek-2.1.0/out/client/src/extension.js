...
function activate(context) {
    ...
    const activeLanguages = config.get('activeLanguages');
    const fileSearchExtensions = config.get('searchFileExtensions');
    ...
    function didOpenTextDocument(document) {
        ...
        if (!clients.has(folder.uri.toString())) {
            Promise.all(fileSearchExtensions.map(type => vscode_1.workspace.findFiles(`**/*${type}`, '')))
                .then(file_searches => {
                let potentialFiles = Array.prototype.concat(...file_searches)
