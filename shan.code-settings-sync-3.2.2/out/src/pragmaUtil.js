...
class PragmaUtil {
     ...
     * @memberof PragmaUtil
     */
    static processBeforeWrite(localContent, newContent, osType, hostName) {
    ...
    static getIgnoredBlocks(content) {
        content = content.replace(/\@sync ignore/g, "@sync-ignore");
        const ignoredBlocks = content.match(this.IgnorePragmaRegExp);
