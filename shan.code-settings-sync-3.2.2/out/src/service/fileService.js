...
class FileService {
    ...
    static GetCustomFile(filePath, fileName) {
        return __awaiter(this, void 0, void 0, function* () {
            const fileExists = yield FileService.FileExists(filePath);
            if (!fileExists) {
                return null;
            ...
            const gistName = FileService.CUSTOMIZED_SYNC_PREFIX + fileName;
            const file = new File(fileName, content, filePath, gistName);
            return file;
        });
    }
    static CreateCustomDirTree(filePath) {
        return __awaiter(this, void 0, void 0, function* () {
            const dir = path.dirname(filePath);
            const fileExists = yield FileService.FileExists(dir);
            if (!fileExists) {
                // mkdir recursively
                yield fs.mkdirs(dir);
            }
    ...
    static ConcatPath(...filePaths) {
        return filePaths.join(path.sep);
    }
}
FileService.CUSTOMIZED_SYNC_PREFIX = "|customized_sync|";
